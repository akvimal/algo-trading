...
// api.ts already uses for market-data - NOT signal-engine's own backend,
// since this isn't its data. signal-engine's own backend has no auth of
// its own (every route is open) - this is a frontend-only gate, same
// convention manual-trading's frontend already uses, added here so the
// Algo Trading tab isn't the one menu left wide open once every sibling
// module requires a login.
...
// Cached alongside the token purely for display - the shell's own top bar
// now shows "logged in as ..." (see shell/index.html), so this no longer
// backs any UI inside this frontend itself, only AuthGate's startup check
// (which sets it, matching every other frontend's convention).
