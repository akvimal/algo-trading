...
- **Rendering** (`htfSetup`, a 2-point overlay `confirmation candle` -> `resolution bar`/now): entry line (dashed; solid once triggered), red stop line, green target line, a translucent red risk box (entry↔stop) and green reward box (entry↔target), a `"15m long · triggered · 2.3R"` label. Resolved setups drawn at ~34% opacity. Multi-timeframe works - entry/SL/target are absolute prices, so a 15m setup renders correctly on a 1m chart.
