...
// Ordered arrays (display/autocomplete order - ConditionsTextEditor.tsx's
// own suggestion lists) backing the Sets below (membership checks,
// parsing) - single source of truth for the grammar either way, so a
// future new interval/field/function only needs adding here once.
export const INTERVALS_LIST = ["1min", "3min", "5min", "15min", "30min", "60min", "daily"] as const;
export const FIELD_NAMES_LIST = ["open", "high", "low", "close", "volume"] as const;
// Bare-keyword terms - candle_body/candle_range plus their short aliases
// (see parseTermBase below) - not function calls, not FIELD_NAMES_LIST.
export const KEYWORD_TERMS_LIST = ["candle_body", "body", "candle_range", "range"] as const;
export const TWO_ARG_FUNCS_LIST = ["sma", "ema", "highest", "lowest"] as const;
export const ONE_ARG_FUNCS_LIST = ["rsi", "cci"] as const;

const VALID_INTERVALS: Set<string> = new Set(INTERVALS_LIST);
const FIELD_NAMES: Set<string> = new Set(FIELD_NAMES_LIST);
const TWO_ARG_FUNCS: Set<string> = new Set(TWO_ARG_FUNCS_LIST);
const ONE_ARG_FUNCS: Set<string> = new Set(ONE_ARG_FUNCS_LIST);
// Checked longest-first so ">=" is never misread as ">" followed by a
// stray "=" - see splitOnOperator. Exported (as OPERATORS_LIST) for
// ConditionsTextEditor.tsx's own autocomplete, which suggests these once
// the left-hand term is already complete.
export const OPERATORS_LIST = [">=", "<=", ">", "<"] as const;
const OPERATORS: Condition["operator"][] = OPERATORS_LIST as unknown as Condition["operator"][];
...
// Exported for ConditionsTextEditor.tsx's own autocomplete - used only to
// check "is whatever's typed so far already a complete, valid term" (e.g.
// to know when to start suggesting a comparison operator instead of more
// term text), never to build a Condition itself outside parseConditionsText.
export function parseTerm(raw: string): Term | string {
  let s = raw.trim();
