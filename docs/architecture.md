...
A **Rule** (`signal-generation`, `signal_generation.rules` table, `app/domain/rule.py`) is a saved, reusable, independently-backtestable definition of *when a signal should fire* — the underlying to watch (or an NSE index-constituent universe), the candle interval, and the condition itself (crossover/breakout/range_breakout — the same 3 types that used to live directly on Strategy, unchanged). A Strategy just **picks** one via `rule_id`, required for every `source_type`. One Rule can back many Strategies (e.g. the same crossover backing both a spot Strategy and an option-spread Strategy on the same underlying) — this is the entire point of the split: separating *what decides a signal fires* from *what happens once it does* (instrument/segment to trade, stop-loss/target, option shape, conflict policies — all still Strategy fields).

For an **external** (webhook) Strategy, the linked Rule carries no evaluable condition at all — `underlying`/`interval`/`rule_config` are all `NULL`, and the row exists purely as a saved name/description reference to the provider's own scan (`provider_rule_name` records the scan's original name if the Rule's own `name` renames it locally). The provider's webhook still fires directly (`?strategy_id=`, unchanged) and still carries its own `action` — the Rule is never evaluated for external strategies, same as before the split; `source_type` on the Rule and on its linked Strategy must match (`validate_rule_link_consistency`, checked at Strategy create/update time against the loaded Rule row), so a webhook strategy can never accidentally point at an in-house-evaluable rule or vice versa.
