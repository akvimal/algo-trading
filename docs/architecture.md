...
**Intraday is now just the Live Chart** (2026-09-03): the Workspace / Trade Checklist / Performance sub-tabs are gone - `IntradayPage.tsx` renders `LiveChartPage` directly, no sub-nav. The chart's own `ChartTradePanel` covers day-to-day order entry + management; intraday performance is slated to move to a future site-wide dashboard. `WorkspacePage.tsx` / `TradeChecklistPage.tsx` / `ManualStatsPage.tsx` stay in the repo, **unrouted** (tree-shaken out of the build - the main JS chunk dropped ~80KB) - they're still the only UI for limit / SL-limit entries, %/previous-candle stop methods, option trailing SL, partial exits, and the trade-review flow, so the code is kept for if any of that is needed again (the underlying `execution` endpoints are all untouched).
...
- The "Order blocks" dropdown is renamed **"Structure ▾"**: detection-timeframe checkboxes (1m/3m/5m/15m/30m/1h) + an "Also show" group (global modifiers over every enabled timeframe). Config persisted to `localStorage` (`manualLiveChartOrderBlocks`) as `{tfs, breakers, fvg, breaks, setups}` - migrates from the old bare `string[]`.
...
- **`structure_state`**: walks the series keeping the most recent *unbroken* pivot high/low. A close beyond one is a break, labelled against the previous break's direction - same = **BOS** (continuation), opposite = **CHoCH** (change of character). Returns the last 8 `StructureEvent`s (`{kind, direction, price, timestamp}`).
...
Fold the "change in OI" data (`market_data.sentiment_history` + `GET /options/sentiment` / `GET /options/oi-summary`) into the live chart. **Only the 4 `SENTIMENT_UNDERLYINGS` have OI** (NIFTY, BANKNIFTY, GOLDM, CRUDEOILM) - hardcode the 4 in the frontend like `OiSummaryPage`'s `PRESETS` already does; for every other symbol the OI controls simply don't render. OI is 5-minute cadence, session hours only, persisted only since ~mid-Aug 2026. Ideas, ranked by value ÷ effort:
...
- **v1.1 - direction lock (2026-09-03).** A checkbox **above** the trade panel ("Trade with the `<interval>` trend only"), rendered by `LiveChartPage` in a `.chart-trade-col` wrapper so it sits at the height of the chart's interval strip, not inside `ChartTradePanel`. Persisted globally in `localStorage["manualChartForceTrendDirection"]`, **default ON** (opt out, not in). When on and the chart-interval's confirmed SMC structure trend is known: an uptrend permits Bullish entries only, a downtrend Bearish only, a ranging trend permits neither (Proceed disabled with a hint) - the disallowed bias button is disabled and the bias snaps to the permitted side, so with the lock on you cannot change direction without unchecking it. The trend is lifted out of `LiveChartPanel` (which already computes `trendByTf` per detection timeframe) via a new `onTrendChange({trend, interval})` prop up to `LiveChartPage`, which holds the `forceTrend` state + the trend and passes `forceTrend`/`intervalTrend`/`chartInterval` down to the panel. It's a soft, client-only discipline aid: it needs Structure detection running on the chart's interval (the default once the "Structure ▾" layer is enabled, since changing the interval re-points detection to it) - with detection off the chip reads "trend n/a" and nothing is blocked. Only new entries are gated; open-position management is untouched. No backend involvement.
- **Deferred - the shared engine (original Phase A/B).** Extracting `WorkspacePage`'s stateful order engine (`placeOrder`/`executeOrder`, the two pending-entry watches, trailing-SL, risk-based sizing, `refreshOpenInstance`) into a `useManualOrder` hook + a `ManualOrderProvider` in `IntradayPage`, and pointing both surfaces at it. Not started - it touches the single most safety-critical file in the frontend and buys the chart panel only the features v1 deliberately skips. Do it when the chart panel needs limit/SL-limit entries or a browser-side target watch (the features that genuinely need the hoisted loop), not before.
- **Deferred - v1 gaps.** Limit / SL-limit entry order types; a browser-side target watch; trailing stop-loss; risk-based auto-sizing; partial square-off; "Range" (needs the neutral-options-strategy backend work - see § "Open questions → credit spreads"); trade review + screenshots (the panel links to the Workspace tab for these). "SL Limit" in the mockup is treated for now as "attach a stop-loss", which the SL field already does.
