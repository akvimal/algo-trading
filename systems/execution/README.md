...
3. An APScheduler job (`app/scheduler.py`), self-contained in this service, fires at `square_off_time` daily: fetches CMP per open symbol from `market-data`, closes each position with signed P&L. `POST /positions/square-off` runs the same logic manually.
