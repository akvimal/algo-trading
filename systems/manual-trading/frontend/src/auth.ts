...
// api.ts already uses for execution/market-data/signal-generation/
// signal-processing. Duplicated from execution/frontend's own auth.ts, not
// shared - see CLAUDE.md's systems/* self-containment rule (matches
// SignalNotifier.tsx's existing copy-not-share precedent).
...
// as ..." line) so it doesn't need its own GET /auth/me round-trip -
// AuthGate's own startup check already fetches the authoritative value.
