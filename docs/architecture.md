...
- `detect_order_blocks` takes the trend and tags each zone `counter_trend` (a demand zone in a downtrend, or supply in an uptrend). Frontend **dims counter-trend zones to ~40%** (fill + border + `" · counter"` label) so the with-trend zones stand out; a per-timeframe trend chip (`▲ up` / `▼ down` / `– range`) shows next to each enabled timeframe in the menu; a "Structure breaks (BOS / CHoCH)" toggle draws each event as a thin horizontal line (green up / red down, CHoCH dashed) via the `htfStructureBreak` custom overlay.
