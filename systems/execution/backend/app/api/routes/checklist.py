...
def get_checklist_items(active_only: bool = False, phase: Optional[str] = Query(default=None), db: Session = Depends(get_db)):
    ...
    that only want one list."""
    return [_item_to_out(r) for r in list_checklist_items(db, active_only=active_only, phase=phase)]


@router.post("/checklist-items")
def create_item(payload: ChecklistItemCreate, db: Session = Depends(get_db)):
    row = create_checklist_item(db, payload.label, payload.phase, payload.segments, payload.sort_order)
    return _item_to_out(row)


@router.put("/checklist-items/{item_id}")
def update_item(item_id: str, payload: ChecklistItemUpdate, db: Session = Depends(get_db)):
    try:
        parsed_id = uuid.UUID(item_id)
    except ValueError:
        raise HTTPException(status_code=404, detail="checklist item not found")
    row = update_checklist_item(
        db, parsed_id, payload.label, payload.phase, payload.segments, payload.sort_order, payload.active
    )
    if row is None:
        raise HTTPException(status_code=404, detail="checklist item not found")
    return _item_to_out(row)
...
def delete_item(item_id: str, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=404, detail="checklist item not found")
    if not delete_checklist_item(db, parsed_id):
        raise HTTPException(status_code=404, detail="checklist item not found")
    return {"deleted": True}
...
def get_pending_review(db: Session = Depends(get_db)):
    ...
    return {"pending": find_pending_manual_review(db)}


@router.get("/daily-checklist")
def get_daily(segment: str = Query(...), db: Session = Depends(get_db)):
    ...
    row = get_daily_checklist(db, settings, segment)
    return _daily_to_out(_today_in_tz(settings.timezone), segment, row)


@router.put("/daily-checklist")
def put_daily(payload: DailyChecklistSubmit, db: Session = Depends(get_db)):
    ...
    today."""
    settings = load_settings(db)
    row = submit_daily_checklist(db, settings, payload.segment, [a.model_dump() for a in payload.answers], payload.notes)
    return _daily_to_out(row.log_date, payload.segment, row)


@router.get("/trading-sessions")
def get_trading_sessions(segment: Optional[str] = Query(default=None), db: Session = Depends(get_db)):
    ...
    return [_session_to_out(r) for r in list_trading_sessions(db, segment)]


@router.post("/trading-sessions/check-in")
def post_check_in(segment: str = Query(...), db: Session = Depends(get_db)):
    ...
    settings = load_settings(db)
    row = check_in_trading_session(db, settings, segment)
    return _session_to_out(row)


@router.post("/trading-sessions/check-out")
def post_check_out(segment: str = Query(...), db: Session = Depends(get_db)):
    ...
    settings = load_settings(db)
    row = check_out_trading_session(db, settings, segment)
    if row is None:
