...
### SMC setup engine - structure primitives shipped (3a-3b, 2026-09-03); setups/persistence scoped

The evolution of Track 1: promote the standalone zone overlay into a full **Smart-Money-Concepts setup engine** - FVGs, breaker blocks and BOS/CHoCH structure state (**shipped, Phase 3a-3b**), then a rejection-confirmed trade idea (entry / SL / target) drawn on the chart, a lifecycle that removes an invalidated setup, and a persisted log of every setup the engine called (**3c-3d, scoped**). Same design as the scoped SMC order-block *Rule* (§ "Open questions" → "SMC order-block rule") applied to the Live Chart as a visual/alerting tool instead of a backtest `RuleConfig`. **Locked in: `market-data`'s `app/domain/order_blocks.py` (`smc.py` in spirit) is the single reference implementation; when the signal-engine SMC Rule is built it *copies* from here** (same "no cross-system import" precedent as `option_templates.py`).
...
- The "Order blocks" dropdown is renamed **"Structure ▾"**: detection-timeframe checkboxes + an "Also show" group (global modifiers over every enabled timeframe). Config persisted to `localStorage` (`manualLiveChartOrderBlocks`) as `{tfs, breakers, fvg, breaks}` - migrates from the old bare `string[]`.
...
**Scoped for 3c (setups)** - each maps to concrete detection over one series:
- **Confirmation (candle pattern at the zone)**: price retests a trend-aligned zone, then - reusing the SMC Rule's settled definition - **mandatory** the candle closes back beyond the zone edge, **plus at least one of** a rejection wick (wick ≥ N× body) or an engulfing of the prior body. Confirmed on the *detection* series (authoritative), same as mitigation today.
- **Entry / SL / target** (SMC Rule's settled scheme): entry = a stop at the rejection candle's own high/low; SL = ATR-buffered beyond the zone's outer (distal) edge (scales per instrument, not a fixed-point buffer); target = `max(post-impulse extreme, prior opposing swing)` for a long (mirror for a short - needs a running impulse extreme tracked from the structure-break candle). Gated on `min_risk_reward` before the setup is emitted - never adjusted to force the ratio.

**Lifecycle**: `pending` (trend-aligned zone identified, waiting for retest) → `confirmed` (rejection candle printed, entry/SL/target computed) → `triggered` (price hit entry) → `hit_target` / `hit_sl` / `invalidated`. **Invalidated** = the zone is closed through before confirmation, OR entry never triggers within N bars of confirmation, OR a CHoCH prints against the setup's direction. On invalidation the engine removes **that setup's** overlays (entry/SL/target lines, risk/reward boxes) - *not* the whole chart (drawings, indicators, other setups stay). An optional "keep invalidated setups greyed" toggle for review.

**Rendering**: entry line (dashed, neutral), SL line (red), target line (green), a translucent red risk box (entry↔SL) and green reward box (entry↔target), a status pill. All in the existing custom-overlay / `groupId` machinery. Multi-timeframe still works - entry/SL/target are absolute prices, so a 15m setup renders correctly on a 1m chart; only the confirmation candle is evaluated on the 15m series.

**Storage** (the "setups should be stored" requirement): a new **append-only `market_data.chart_setups` table** (the second sanctioned exception to market-data-is-cache-only, same precedent as `sentiment_history`) - one row per setup, updated as its status advances, carrying the zone, entry/SL/target, R:R, and final outcome. Written by a **scheduler job** (mirroring `_record_sentiment_history`) that runs the engine every N minutes for a watched-symbol set (`SENTIMENT_UNDERLYINGS` as the default, or a small configurable list) - so history accrues without the frontend POSTing anything. The Live Chart's own `GET /chart-setups?exchange&symbol&interval&from&to` is a stateless live recompute for whatever the user is viewing; the *stored* setups are the scheduler's. Volume is tiny (a few setups per symbol per day - one row each, unlike the deferred per-strike OI history's 40-80 rows/snapshot). Feeds a future "Setups" review panel (win-rate, avg-R, like `ManualStatsPage` but for algo-called setups).

**Execution bridge, not execution**: v1 is analysis/alerting only - same reason the SMC Rule's v1 is backtest-only (`resolved-order.schema.json` has no absolute-price SL/target field, and none of execution's three live `stop_loss_method` values reproduce a zone-edge stop). A "→ Manual tab" button pre-fills `WorkspacePage`'s order form (action, symbol, entry as the Limit price, `stop_loss_price`, and the browser-side target watch) - all fields `POST /positions/manual` already accepts, no contract change.
...
- **3c - setups**: confirmation + entry/SL/target + lifecycle + rendering + the "→ Manual tab" bridge.
- **3d - persistence + review**: the `chart_setups` table, the scheduler job, the Setups review panel.

**Caveats**: SMC definitions are subjective - ship with locked defaults (the SMC Rule entry already settled several), expose tuning later. State-heavy (trend flag, running impulse extreme, per-zone lifecycle) vs today's stateless recompute - a full-series replay per call is still fine for the Live Chart (a few thousand bars) and the scheduler. "Clear the chart on invalidation" means the *setup's* overlays, never the user's drawings/indicators.
