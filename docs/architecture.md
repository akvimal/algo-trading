...
### SMC setup engine - FVG + breakers shipped (3a, 2026-09-03); setups/lifecycle/persistence scoped

The evolution of Track 1: promote the standalone zone overlay into a full **Smart-Money-Concepts setup engine** - FVGs and breaker blocks (**shipped, Phase 3a**), then structure context, a rejection-confirmed trade idea (entry / SL / target) drawn on the chart, a lifecycle that removes an invalidated setup, and a persisted log of every setup the engine called (**3b-3d, scoped**). Same design as the scoped SMC order-block *Rule* (§ "Open questions" → "SMC order-block rule") applied to the Live Chart as a visual/alerting tool instead of a backtest `RuleConfig`. **Locked in: `market-data`'s `app/domain/order_blocks.py` (`smc.py` in spirit) is the single reference implementation; when the signal-engine SMC Rule is built it *copies* from here** (same "no cross-system import" precedent as `option_templates.py`).
...
- The "Order blocks" dropdown is renamed **"Structure ▾"**: detection-timeframe checkboxes + an "Also show" group (Breakers, Fair value gaps - global modifiers over every enabled timeframe). Config persisted to `localStorage` (`manualLiveChartOrderBlocks`) as `{tfs, breakers, fvg}` - migrates from the old bare `string[]`.

**Scoped for 3b (structure state)**: a running swing-break sequence → a trend flag (uptrend/downtrend). BOS = same-direction break (continuation), CHoCH = opposite (potential reversal); BOS-then-CHoCH-same-side flips the trend. The heaviest state and the same simplification the SMC Rule entry locks in. Needed so the engine only takes setups *with* the trend (longs at demand in an uptrend, shorts at supply in a downtrend).
...
**Phasing**:
- **3a - FVG + breaker overlays** - ✅ shipped 2026-09-03 (visual only, stateless, per-timeframe).
- **3b - structure state**: swing detection + BOS/CHoCH + trend flag; tag/dim counter-trend zones; optional break markers. The state-management step.
