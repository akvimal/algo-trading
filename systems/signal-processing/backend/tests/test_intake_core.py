...
import app.domain.intake.core as core
...
def test_resolve_and_finalize_signal_publishes_use_margin_true(monkeypatch):
    ...
    monkeypatch.setattr(
        core,
        "resolve",
        lambda signal: ResolvedOrderDraft(horizon="positional", instrument_type="spot", segment="NSE", use_margin=True),
    )
    published = {}
...
def test_resolve_and_finalize_signal_publishes_use_margin_false_by_default(monkeypatch):
    ...
    monkeypatch.setattr(
        core,
        "resolve",
        lambda signal: ResolvedOrderDraft(horizon="intraday", instrument_type="spot", segment="NSE"),
    )
    published = {}
    monkeypatch.setattr(core, "publish_resolved_order", lambda payload: published.update(payload))
