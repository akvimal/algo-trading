...
def test_run_recreates_the_consumer_group_every_iteration_not_just_once(monkeypatch):
    """Regression test for a real incident (2026-08-31, on the VPS deploy):
    Redis lost signals.pending_resolution's stream/consumer group while
    this thread kept running uninterrupted (a Redis restart with no
    persistence, or its container/volume being recreated) -
    _ensure_group() used to run only once, before the loop, so from that
    point on the thread NOGROUP-looped on every xautoclaim/xreadgroup call
    forever, with no way to recover short of restarting the whole
    service. Fixed by re-running _ensure_group() every iteration instead
    (a cheap no-op via BUSYGROUP once the group already exists) - this
    proves it's actually called more than once per run(), not just at
    startup."""
    ensure_group_calls = []
