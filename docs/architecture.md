...
### `option_fixed_lots`: a narrow, deliberate exception to "no quantity on Strategy"

Strategy carries no quantity/capital field on principle (see the section header above) — with one narrow, options-only exception: `Strategy.option_fixed_lots` (optional, `instrument_type='option'` only, harmlessly ignored otherwise — same convention as `option_position_style`/`option_strike_moneyness`/`option_sl_scope`). When set, `open_option_group()` trades exactly `option_fixed_lots * lot_size` and skips `compute_quantity`/`compute_risk_based_quantity` entirely — it takes precedence over stop-loss-based (risk-%) sizing, even when a stop-loss is also configured. The stop-loss/target *prices* are still computed and stored as normal; only their role in sizing is bypassed. This isn't a reversal of "sizing is a capital/risk decision that lives in execution" — `risk_per_trade_pct` stays exactly where it was, on `execution.accounts` — it's "some options strategies want a fixed contract count regardless of capital," a real, distinct sizing mode, not a capital exposure decision signal-generation is making on execution's behalf. The insufficient-balance check still runs against the fixed count's real cost (`net_debit * lot_size * option_fixed_lots`), not a 1-lot minimum — a fixed count that's genuinely unaffordable against `current_balance` still rejects cleanly, same "paper trading still respects the simulated balance" reasoning every other rejection case in `option_position_manager.py` already has.
...
### Market regime filter: 5 independent Indicator types, gating on the Rule, not the Strategy

A shared trading-analysis document recommended classifying market regime (`UPTREND`/`DOWNTREND`/`RANGE`/`TRANSITION`) from confirmed swing structure (HH/HL vs LH/LL), Efficiency Ratio, ADX/DMI, and ATR-normalized EMA slope — combined by AND-conditions, not voted equally — and using it as supporting evidence alongside a primary trading signal. `app/domain/regime.py` still implements all of that math (`classify_regime`/`direction_confirmed`/`RegimeParams`, unchanged, self-contained, tested) but it's no longer what live/backtest code actually calls to gate a signal — the gate was originally a single `Strategy.regime_filter_enabled`/`regime_filter_checks` toggle (one shared, non-configurable `RegimeParams` bundle), then reworked into **5 independent, creatable, reusable `Indicator` types** instead: `structure`, `efficiency_ratio`, `adx`, `dmi_direction`, `ema_slope` (alongside the pre-existing `rsi`) — each with its own Pydantic params model in `app/domain/rule.py` (e.g. `AdxParams = {period, trend_threshold}`) and its own independently-computed `regime.check_*` function (`check_structure`/`check_efficiency_ratio`/`check_adx`/`check_dmi_direction`/`check_ema_slope`, each a decomposition of the original `_direction_checks` combiner, reusing the exact same tested math — some redundant computation between e.g. `adx`/`dmi_direction`, both needing their own ADX/DMI pass, accepted since this only runs when a rule's own trigger has already fired, not a hot path). `IndicatorType` is `Literal["rsi", "structure", "efficiency_ratio", "adx", "dmi_direction", "ema_slope"]`; `REGIME_INDICATOR_TYPES` names the 5 regime ones. `app/domain/indicators.py` gained `evaluate_regime_indicator(indicator_type, params, candles, bias) -> Optional[bool]` (dispatches to the matching `regime.check_*`) and `regime_indicator_warmup(indicator_type, params) -> int`, mirroring `compute_indicator`/`indicator_warmup`'s own dispatch shape one level up.
