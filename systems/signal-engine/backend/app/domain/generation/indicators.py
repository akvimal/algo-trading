...
if TYPE_CHECKING:
    # Type-checking only - app.domain.generation.rules imports this module (for
    ...
    # regime_indicator_warmup below import app.domain.generation.regime lazily
    # (function-local) for the same reason - regime.py itself imports
    # Bias/CandleClose from app.domain.generation.rules.
...
def compute_indicator_signal(indicator_type: str, params: dict, candles: "list[CandleClose]") -> list[Optional[float]]:
    ...
    if indicator_type == "supertrend":
        from app.domain.generation.regime import compute_supertrend
...
def evaluate_regime_indicator(indicator_type: str, params: dict, candles: "list[CandleClose]", bias: "Bias") -> Optional[bool]:
    ...
    from app.domain.generation.regime import (
        check_adx,
        check_dmi_direction,
        check_ema_slope,
