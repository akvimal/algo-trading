...
def test_crypto_bucket_is_one_call_covering_all_three_symbols(monkeypatch):
    ...
    assert [a.title for a in btc] == ["BTC and ETH both rally"]
    assert btc[0].sentiment_score == 0.5
    assert [a.title for a in eth] == ["BTC and ETH both rally"]
    assert eth[0].sentiment_score == 0.2
    assert [a.title for a in sol] == ["SOL breaks out"]
...
def test_search_underlying_uses_its_own_keyword(monkeypatch):
    ...
    articles = news.get_news("NIFTY")

    assert len(calls) == 1
    assert calls[0]["search"] == "Nifty 50"
    assert articles[0].title == "Nifty 50 hits record high"
...
def test_stale_cache_is_served_when_refresh_fails(monkeypatch):
    ...
    assert first[0].title == "Gold steady ahead of Fed"

    # Expire the cache, then make the next refresh fail - should fall back
    # to the stale copy instead of raising.
    underlying, (articles, _fetched_at) = "GOLDM", news._cache["GOLDM"]
    news._cache[underlying] = (articles, 0.0)
    ...
    assert second[0].title == "Gold steady ahead of Fed"
